...
COMMENT ON COLUMN active_watchers.station_id IS 'ID de la estación de Electromaps';
COMMENT ON COLUMN active_watchers.station_name IS 'Nombre legible de la estación';
COMMENT ON COLUMN active_watchers.status IS 'active=vigilando, completed=llamada realizada, cancelled=cancelado manualmente';
COMMENT ON COLUMN active_watchers.last_connector_states IS 'Snapshot JSON del estado de los conectores en la última consulta';
